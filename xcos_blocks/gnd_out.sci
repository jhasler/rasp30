function [x,y,typ]=gnd_out(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
    ...
    case 'set' then
        ...
        while %t do
            [ok,in_out_num,mblif_num,fix_loc,exprs]=getvalue('Set GND_o Parameters',['Number of gnd_o blocks';'Macro-blif Number';'Fix_location'],list('vec',-1,'vec',-1,'vec',-1),exprs)
