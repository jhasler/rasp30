function [x,y,typ]=common_source(job,arg1,arg2)
    ...
    case 'set' then
        ...
        while %t do
            [ok,num_of_blk,mblif_num,common_source_ibias,fix_loc,exprs]=scicos_getvalue('New Block Parameter',['num_of_blk';'mblif_num';'common_source_ibias';'fix_loc'],list('vec',-1,'vec',-1,'vec',-1,'vec',-1),exprs);
            if ~ok then break,end
            if ok then
                model.ipar=[num_of_blk,mblif_num];
                model.rpar=[common_source_ibias,fix_loc'];
    ...
    case 'define' then
        num_of_blk=1;
        mblif_num=0;
        common_source_ibias=50e-9;
        fix_loc=[0;0;0];
        model=scicos_model();
        model.sim=list('common_source_c',5);
        model.in=[-1];
        model.in2=[-1];
        model.intyp=[-1];
        model.out=[-1];
        ...
        exprs=[sci2exp(num_of_blk);sci2exp(mblif_num);sci2exp(common_source_ibias);sci2exp(fix_loc)];
        gr_i=['text=[''common_source''];';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'');']
