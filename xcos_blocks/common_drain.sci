function [x,y,typ]=common_drain(job,arg1,arg2)
    ...
    case 'set' then
        ...
        while %t do
            [ok, in_out_num, common_drain_fgswc_ibias, fix_loc,exprs]=scicos_getvalue('New Block Parameter',['number of blocks'; 'common_drain_fgswc_ibias';'Fix_location'],list('vec',-1, 'vec', -1,'vec',-1),exprs);
            if ~ok then break,end
            if ok then
                model.ipar=[in_out_num];
                model.rpar=[common_drain_fgswc_ibias ,fix_loc'];
    ...
    case 'define' then
        in_out_num=1
        common_drain_fgswc_ibias=[50e-9];
        fix_loc=[0;0;0]
        model=scicos_model()
        ...
        exprs=[sci2exp(in_out_num);sci2exp(common_drain_fgswc_ibias);sci2exp(fix_loc)]
        gr_i=['txt=[''common_drain''];';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'');']
