...
function [x,y,typ]=FG_NFET_mod(job,arg1,arg2)
x=[];y=[];typ=[];
select job
case 'plot' then
...
case 'set' then
  ...
  model=arg1.model;
  while %t do
    [ok,K,UT,Ith,sig,VT0,Vfg0,C1,exprs]=scicos_getvalue('Set NMOS Transistor parameters',..
	['Kappa ';..
	 'UT [V]';..
	 'Ith [A]';..
	 'sigma';..
...
case 'define' then
  ...
  model.sim='FG_NFET_mod'
  model.blocktype='c'
  model.dep_ut=[%t %f]
  mo=modelica()
  mo.model='FG_NFET_mod';
  mo.outputs=['D';'B';'S']
  mo.inputs='G';
